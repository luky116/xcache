	...
	for _, line := range strings.Split(text, "\n") {
		...
		if key := strings.TrimSpace(kv[0]); key == "binlog_offset" {
			...
			if len(binlog_offset) == 2 {
				...
				offset, err = strconv.Atoi(binlog_offset[1])
				if err != nil {
					return 0, 0, errors.Trace(err)
				}
			} else {
				return 0, 0, errors.New("binlog offset parse err")
			}
			break
		}
	}
...
func (c *Client) SlowLog(cmd string) (string, error) {
	cmd_arr := strings.Split(cmd, " ")
	params_arr := cmd_arr[1:]
	params := make([]interface{}, len(params_arr))
	for i, v := range params_arr {
	...
	text, err := c.Do(cmd_arr[0], params...)
	if err != nil {
		return "", errors.Trace(err)
	}

	resp, err := SlowLogParse(text)
	if err != nil {
		return "", errors.Trace(err)
	}
...
func SlowLogParse(resp interface{}) (string, error) {
	resp_arr, err := redigo.Values(resp, nil)
	if err != nil {
		return "", errors.Trace(err)
	}
	response := ""
	...
	for i, v := range resp_arr {
		...
		response += fmt.Sprintf(format_str, i+1) + ")" + " 1) (integer) " + strconv.FormatInt(slow_log[0].(int64), 10) + "\r\n"
		response += resp_preffix + "  2) (integer) " + strconv.FormatInt(slow_log[1].(int64), 10) + "\r\n"
		response += resp_preffix + "  3) (integer) " + strconv.FormatInt(slow_log[2].(int64), 10) + "\r\n"
		response += resp_preffix + "  4) "

		cmd, err := redigo.Strings(slow_log[3], nil)
		if err != nil {
			return "", errors.Trace(err)
		}
		for j := 0; j < len(cmd); j++ {
