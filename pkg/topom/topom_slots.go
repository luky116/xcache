...
func (s *Topom) SlotRemoveActionAll() error {
	...
	for _, m := range ctx.slots {
		if m.Action.State == models.ActionNothing || m.Action.State != models.ActionPending {
			continue
		}
		defer s.dirtySlotsCache(m.Id)
...
func (s *Topom) SlotActionPrepareFilter(accept, update func(m *models.SlotMapping) bool) (int, bool, error) {
	...
	var m = func() *models.SlotMapping {
		...
		if s.action.disabled == 1 || s.action.disabled == 2 {
			return nil
		}
...
func (s *Topom) SlotActionComplete(sid int) error {
	...
	case models.ActionNothing:
		...
		log.Warnf("slot-[%d] maybe migrated to target", m.Id)
...
func (s *Topom) newSlotActionExecutor(sid int) (func(db int) (remains int, nextdb int, err error), error) {
	...
	case models.ActionMigrating:
		...
		if m.GroupId == m.Action.TargetId {
			return func(int) (int, int, error) {
				return 0, -1, nil
			}, nil
		}
...
func (s *Topom) ExpansionMgrtSlots(slots []int, gid int, must bool) error {
	ctx, err := s.newContext()
	if err != nil {
		return err
	}
	log.Warnf("start ExpansionMgrtSlots: to group %d", gid)
...
func (s *Topom) isSlotsActionFinsh(slots []int) bool {
	s.mu.Lock()
	defer s.mu.Unlock()
	ctx, err := s.newContext()
	if err != nil {
...
func (s *Topom) getSlotsList(slots string, srcGid, dstGid int) ([]int, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	slotsList := strings.Split(slots, ",")
	for _, slotPlan := range slotsList {
		slotPlanPair := strings.Split(slotPlan, "-")
		if len(slotPlanPair) == 1 {
		...
		for sid := beg; sid <= end; sid++ {
			m, err := ctx.getSlotMapping(sid)
			if err != nil {
				return slice, errors.New("get slotsinfo failed")
			}
			if m.Action.State != models.ActionNothing {
				return slice, errors.Errorf("have slots action running in plan: %s", slots)
			}
			if m.GroupId != srcGid {
				return slice, errors.Errorf("invalid plan: have slots not on src group: %s", slots)
			}
			slice = append(slice, m.Id)
		}
	}
	return slice, nil
}
