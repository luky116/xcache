...
func (s *Topom) ResyncSentinels() error {
	...
	return s.ResyncSentinelsNoLock()
}

func (s *Topom) ResyncSentinelsNoLock() error {
	ctx, err := s.newContext()
	if err != nil {
		return err
	}
...
func (s *Topom) SentinelRemoveGroupsAll() error {
	s.mu.Lock()
	defer s.mu.Unlock()
	ctx, err := s.newContext()
	if err != nil {
	...
	//p.OutOfSync = false
	//return s.storeUpdateSentinel(p)
	//此时sentinel没有监控集群中的所有组，可以提醒用户后续进行同步操作
	return nil
}
...
func (s *Topom) SentinelRemoveGroupNoLock(gid int) error {
	ctx, err := s.newContext()
	if err != nil {
		return err
	}
